...
def patch_file(filepath):
  ...
  // Helper function to strip HTML for preview"""
    ...
    # The existing rendering starts at: <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '1.5rem' }}>
    ...
    # 8. Add Share Button next to Edit / Delete
    old_actions = """<div style={{ display: 'flex', gap: '0.5rem' }}>
                    {(role === 'admin' || (role === 'teacher' && post.author === authorName)) && (
                      <>
                        <button className="icon-btn" onClick={(e) => openEditModal(post, e)} style={{ color: 'var(--primary-color)' }} title="កែប្រែ">
                          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                        </button>
                        <button className="icon-btn" onClick={(e) => handleDelete(post.id, e)} style={{ color: 'var(--danger-color)' }} title="លុប">
    ...
    new_actions = """<div style={{ display: 'flex', gap: '0.5rem' }}>
                    <button className="icon-btn" onClick={(e) => handleShare(e, post.postCode, post.id)} style={{ color: 'var(--primary-color)' }} title="Share">
    ...
    old_title_input = """<div className="form-group">
                ...
                <input 
                  ...
                  placeholder="វាយបញ្ចូលចំណងជើងមេរៀន..."
                  autoFocus
                />
              </div>"""
    new_title_input = """<div className="form-group" style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
                ...
                <div style={{ width: '150px' }}>
                  <label style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>លេខកូដ (ID)</label>
                  <input 
                    type="text" 
                    className="input-field" 
                    value={postCodeField}
