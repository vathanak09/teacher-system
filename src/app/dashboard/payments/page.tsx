...
export default function PaymentsPage() {
  ...
  const [isDueStudentsModalOpen, setIsDueStudentsModalOpen] = useState(false);
  const [isPaidStudentsModalOpen, setIsPaidStudentsModalOpen] = useState(false);
    ...
    <>
      ...
      <div className="flex-between" style={{ marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
          <button onClick={() => setIsDueStudentsModalOpen(true)} className="btn" style={{ background: 'var(--main-bg)', color: '#f59e0b', border: '1px solid #fcd34d', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>
            សិស្សដែលត្រូវបង់ប្រាក់
          </button>
          <button onClick={() => setIsPaidStudentsModalOpen(true)} className="btn" style={{ background: 'var(--main-bg)', color: '#10b981', border: '1px solid #6ee7b7', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>
            សិស្សដែលបានបង់ប្រាក់រួច
          </button>
          <button onClick={() => window.print()} className="btn" style={{ background: 'var(--main-bg)', border: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
      ...
      {isDueStudentsModalOpen && (
        <div className="animate-fade-in" onClick={() => setIsDueStudentsModalOpen(false)} style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)' }}>
          <div className="glass-panel animate-slide-up" onClick={e => e.stopPropagation()} style={{ background: 'var(--modal-bg)', padding: '2rem', borderRadius: '12px', width: '90%', maxWidth: '800px', maxHeight: '80vh', overflowY: 'auto' }}>
            <h2 style={{ marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#f59e0b" strokeWidth="2"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>
              បញ្ជីសិស្សដែលត្រូវបង់ប្រាក់
            </h2>
            <div style={{ display: 'grid', gap: '0.5rem' }}>
              {augmentedStudents.filter(s => s.statusInfo.code === 'due_soon' || s.statusInfo.code === 'overdue').map(student => (
                <div key={student.id} style={{ padding: '1rem', border: '1px solid var(--border-color)', borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <div style={{ fontWeight: 600 }}>{student.fullName} ({student.studentId})</div>
                    <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>ថ្នាក់៖ {student.className}</div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: '0.9rem', color: student.statusInfo.color, fontWeight: 600 }}>{student.statusInfo.label}</div>
                    <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>ថ្ងៃត្រូវបង់៖ {student.nextPaymentDate || student.enrollDate || 'មិនបញ្ជាក់'}</div>
                  </div>
                </div>
              ))}
              {augmentedStudents.filter(s => s.statusInfo.code === 'due_soon' || s.statusInfo.code === 'overdue').length === 0 && (
                <div style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-secondary)' }}>មិនមានសិស្សដែលត្រូវបង់ប្រាក់ទេ</div>
              )}
            </div>
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '1.5rem' }}>
              <button onClick={() => setIsDueStudentsModalOpen(false)} className="btn" style={{ background: 'var(--main-bg)', border: '1px solid var(--border-color)' }}>បិទ</button>
            </div>
          </div>
        </div>
      )}

      {isPaidStudentsModalOpen && (
        <div className="animate-fade-in" onClick={() => setIsPaidStudentsModalOpen(false)} style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)' }}>
          <div className="glass-panel animate-slide-up" onClick={e => e.stopPropagation()} style={{ background: 'var(--modal-bg)', padding: '2rem', borderRadius: '12px', width: '90%', maxWidth: '800px', maxHeight: '80vh', overflowY: 'auto' }}>
            <h2 style={{ marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              បញ្ជីសិស្សដែលបានបង់ប្រាក់រួច
            </h2>
            <div style={{ display: 'grid', gap: '0.5rem' }}>
              {augmentedStudents.filter(s => s.statusInfo.code === 'paid').map(student => (
                <div key={student.id} style={{ padding: '1rem', border: '1px solid var(--border-color)', borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <div style={{ fontWeight: 600 }}>{student.fullName} ({student.studentId})</div>
                    <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>ថ្នាក់៖ {student.className}</div>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: '0.9rem', color: student.statusInfo.color, fontWeight: 600 }}>{student.statusInfo.label}</div>
                    <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>ថ្ងៃត្រូវបង់បន្ទាប់៖ {student.nextPaymentDate || 'មិនបញ្ជាក់'}</div>
                  </div>
                </div>
              ))}
              {augmentedStudents.filter(s => s.statusInfo.code === 'paid').length === 0 && (
                <div style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-secondary)' }}>មិនមានសិស្សដែលបានបង់ប្រាក់រួចទេ</div>
              )}
            </div>
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '1.5rem' }}>
              <button onClick={() => setIsPaidStudentsModalOpen(false)} className="btn" style={{ background: 'var(--main-bg)', border: '1px solid var(--border-color)' }}>បិទ</button>
            </div>
          </div>
        </div>
      )}
