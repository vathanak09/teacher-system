...
export default function DashboardPostViewPage(props: { params: Promise<{ code: string }> }) {
  ...
  return (
    <div className="page-container animate-fade-in" style={{ maxWidth: '1000px', margin: '0 auto' }}>
      ...
      {/* Header */}
      <div className="glass-panel dashboard-header-panel" style={{ position: 'relative' }}>
        ...
        <button 
          onClick={handleLike} 
          style={{ position: 'absolute', top: '1rem', right: '1rem', background: post.likes?.includes(userId) ? 'rgba(244, 63, 94, 0.1)' : 'var(--main-bg)', border: '1px solid var(--border-color)', cursor: 'pointer', color: post.likes?.includes(userId) ? '#f43f5e' : 'var(--text-secondary)', padding: '0.4rem', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s ease' }}
        ...
        <div style={{ display: 'flex', gap: '0.6rem', flexWrap: 'wrap', alignItems: 'center', color: 'var(--text-secondary)', fontSize: '0.85rem', marginBottom: '1rem' }}>
          {post.postCode && (
            <div style={{ background: 'var(--accent-primary)', color: 'white', padding: '0.2rem 0.5rem', borderRadius: '6px', fontWeight: 'bold', fontSize: '0.8rem' }}>
              ID: {post.postCode}
            </div>
          )}
      ...
      {/* Content */}
      <div id="post-content-area" className="glass-panel post-content-container" style={{ position: 'relative', overflowY: 'auto', overflowX: 'auto', background: 'var(--main-bg)' }}>
          ...
          {desktopMode && (
            <>
              <button 
                ...
                className="btn" 
                style={{ padding: '0.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--card-bg)', border: '1px solid var(--border-color)', borderRadius: '8px', boxShadow: '0 2px 5px rgba(0,0,0,0.1)' }}
